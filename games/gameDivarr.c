...
static void update() {
  ...
  if (input.isJustPressed) {
    ...
    if (sc == 0) {
      ...
      s->cc = 0;
      s->isAlive = true;
      shotIndex = wrap(shotIndex + 1, 0, DIVARR_MAX_SHOT_COUNT);
    } else {
      play(SELECT);
      COUNT_IS_ALIVE(shots, sc);
  ...
  if (fallingAddingTicks < 0) {
    ...
    fallingIndex = wrap(fallingIndex + 1, 0, DIVARR_MAX_FALLING_COUNT);
